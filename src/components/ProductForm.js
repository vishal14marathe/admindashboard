...
const inputClass =
    'w-full px-3 py-2 border border-slate-300 rounded-lg text-slate-900 placeholder-slate-400 bg-white focus:border-blue-500 focus:ring-2 focus:ring-blue-100 transition outline-none disabled:bg-slate-100 disabled:text-slate-400';
...
export default function ProductForm({ initialValues, onSubmit, submitting, submitLabel = 'Save' }) {
    ...
    const handleImageChange = (e) => {
        ...
        if (file.size > MAX_MB * 1024 * 1024) {
            setImageError(`Image must be smaller than ${MAX_MB} MB`);
            return;
        }
    ...
    return (
        <form
            ...
            className="space-y-5 max-w-xl bg-white border border-slate-200 rounded-2xl p-6 shadow-sm"
        >
            {/* Image upload */}
            <div>
                ...
                {form.thumbnail ? (
                    ...
                    <label
                        ...
                        className="flex flex-col items-center justify-center w-40 h-40 border-2 border-dashed border-slate-300 rounded-xl cursor-pointer hover:border-blue-400 hover:bg-blue-50/40 transition text-slate-500"
                    >
            ...
            <button
                type="submit"
                disabled={submitting}
                className="w-full bg-blue-600 hover:bg-blue-700 text-white font-medium py-2.5 rounded-lg shadow-sm transition disabled:opacity-60 disabled:cursor-not-allowed"
            >
